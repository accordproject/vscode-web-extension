...
export function getConcertoModelTemplate(documents: Documents, promptEmbedding: any, provider: any, promptConfig: any): Array<{ content: string, role: string }> {
    ...
    if (previousError) {
        const userMessage = `Fix the following errors in previously outputed cto model: ${previousError.map((e: any) => e.message).join('. ')}. Don't put comments in the code and remove all the previous comments present in the code.`;
        messageArray.push({
            content: userMessage,
            role: "user"
        });
